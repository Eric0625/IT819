...
N = 2 if isDEBUG else 50	# number of epochs
...
def extract_data(data):
	...
	# for i in folds:
	features,scores,queries = data#S[i]
...
def get_NDCG(q, model_sco, phase, idcg, only_total=False):	# evaluation metric
	ndocs = len(model_sco)
	...
	for i in range(ndocs):
		if rank[i] < 10 or idcg == IDCGall:			
			B += A[i,i]*(1.954 if idcg == IDCG10 else 1.1)	# this is your basic DCG			
	...
	A /= idcg[phase][q]	# normalise by ideal DCG
	B /= idcg[phase][q]	# normalise by ideal DCG
...
web10kpath = '/Users/user/Documents/ML/ndcg/MSLR-WEB10K/Fold1/'
# for i in range(5):
# 	S.append(get_data('./MSLR-WEB10K/S'+str(i+1)+'.txt'))
train_data = get_data(web10kpath+('strain.txt' if isDEBUG else 'train.txt'))
test_data = get_data(web10kpath + ('stest.txt' if isDEBUG else 'test.txt'))
vali_data = get_data(web10kpath + ('svali.txt' if isDEBUG else 'vali.txt'))
fea,sco,que,M,Q = {},{},{},{},{}
fea['trn'],sco['trn'],que['trn'],M['trn'],Q['trn'] = extract_data(train_data)#extract_data('trn',[1,2,3])
fea['val'],sco['val'],que['val'],M['val'],Q['val'] = extract_data(vali_data)#extract_data('val',[4])
fea['tst'],sco['tst'],que['tst'],M['tst'],Q['tst'] = extract_data(test_data)#extract_data('tst',[5])
print('data loaded')

# precalculate normalising factor for NDCG@10 and NDCG@all
IDCG10 = {'trn':[],'val':[],'tst':[]}
IDCGall = {'trn':[],'val':[],'tst':[]}
for x in sco['trn']:
	top_ten = np.sort(x)[-1:-11:-1]
	top_all = np.sort(x)[-1::-1]
	IDCG10['trn'].append(np.sum((2**top_ten - 1)/np.log2(np.arange(len(top_ten)) + 2)))
	IDCGall['trn'].append(np.sum((2**top_all - 1)/np.log2(np.arange(len(top_all)) + 2)))
for x in sco['val']:
	top_ten = np.sort(x)[-1:-11:-1]
	top_all = np.sort(x)[-1::-1]
	IDCG10['val'].append(np.sum((2**top_ten - 1)/np.log2(np.arange(len(top_ten)) + 2)))
	IDCGall['val'].append(np.sum((2**top_all - 1)/np.log2(np.arange(len(top_all)) + 2)))
for x in sco['tst']:
	top_ten = np.sort(x)[-1:-11:-1]
	top_all = np.sort(x)[-1::-1]
	IDCG10['tst'].append(np.sum((2**top_ten - 1)/np.log2(np.arange(len(top_ten)) + 2)))
	IDCGall['tst'].append(np.sum((2**top_all - 1)/np.log2(np.arange(len(top_all)) + 2)))
...
for n in range(N):
	...
	for q in range(Q[phase]):
		...
		NDCG, increment = get_NDCG(q, model_sco[q], phase, idcg=IDCG10)
...
for q in range(Q[phase]):	# get final NDCG score
	if not IDCG10[phase][q]:
		NDCG_train[N] += 1
		continue
	NDCG_train[N] += get_NDCG(q, model_sco[q], phase, only_total=True, idcg=IDCG10)
NDCG_train /= Q[phase]
...
### PREDICT ###
phase='tst'
NDCG_test10 = np.zeros(N+1)
NDCG_testall = np.zeros(N+1)
for n in range(N+1):
	...
	for q in range(Q[phase]):	# get NDCG scores
		if not IDCG10[phase][q]:
			...
			NDCG_testall[n] += 1
			continue
		NDCG_test10[n] += get_NDCG(q, model_sco[q], phase, only_total=True, idcg=IDCG10)
		NDCG_testall[n] += get_NDCG(q, model_sco[q], phase, only_total=True, idcg=IDCGall)
	if n==N: break
NDCG_test10 /= Q[phase]
NDCG_testall /= Q[phase]
print('ndcg@10 is', np.mean(NDCG_test10))
print('ndcg@all is', np.mean(NDCG_testall))
### PLOTS ###
