...
web10kpath = '/Users/aryawang/Documents/ML/ndcg/MSLR-WEB10K/Fold1/'
# for i in range(5):
#     S.append(get_data(web10kpath + 'Fold'+str(i+1)+'.txt'))
...
train_fea,train_sco,train_que,M_train,Q_train,Q_id_train,relevance_train = extract_data(train_data)#extract_data('train',[1,2,3,4])
#vali_fea, vali_sco, vali_que, M_vali, Q_vali,Q_id_vali, relevance_valid= extract_data('vali',[3])
test_fea, test_sco, test_que, M_test, Q_test, Q_id_test,relevance_test  = extract_data(test_data)#extract_data('test',[5])
print('data loaded')
...
def optimize_cost(x, relevance_labels, learning_rate, n_hidden, n_layers):
    ...
    cost = tf.nn.sigmoid_cross_entropy_with_logits(labels=pairwise_o_ij, logits=targets)
...
def get_NDCG(ranks,scores,level = 10):
    "Get NDCG. Takes in outputs from all docs after being passed through net and takes top 10,"
...
def get_random_NDCG(ranks,scores,level = 10):
    ...
    for i in range(len(ranks)):
        if hasNoLimit:
            top_true_scores.append(list(reversed(np.argsort(scores[i]).tolist())))
        else:
            top_true_scores.append(list(reversed(np.argsort(scores[i])[-level:].tolist())))
    top_rels = []
    top_true_rels = []
    #print(len(top_scores))
    for i in range(len(scores)):
        if hasNoLimit:
            top_rels.append(list(ranks[i]))
        elif len(ranks[i])<level:
            top_rels.append(list(ranks[i][:len(ranks[i])]))
        else:
            top_rels.append(list(ranks[i][:level]))            
        top_true_rels.append(list(scores[i][top_true_scores[i]])) 
    ndcg_q = []
    #pdb.set_trace()
    for i in range(len(top_rels)):
        ...
        fc = 1.22 if hasNoLimit else 3.9
        if len(top_rels[i])<level or hasNoLimit: 
            level = len(top_rels[i])            
        #print(level)   
        for j in range(level):
            current_dcg += fc*((2**top_rels[i][j]) - 1)/np.log(j+1+1)
            current_ideal+= ((2**top_true_rels[i][j]) - 1)/np.log(j+1+1)
            #print(top_rels[i][j])
            #print(current_dcg)
        if  current_ideal != 0:
...
for i in range(50):
    ...
    nd_list10= (get_random_NDCG(random_rank,all_sco[1]))
    nd_listall = (get_random_NDCG(random_rank,all_sco[1], level=-1))    
    list_nds10.append(np.mean(nd_list10))
    list_ndsall.append(np.mean(nd_listall))
    ERR = np.mean(np.nan_to_num(get_ERR(random_rank,all_sco[1])))
    #ndcg_list.append(nd)
    #err_list.append(ERR)
print('NDCG vals fro top 10 = {}'.format(np.mean(list_nds10)))
print('NDCG vals fro all = {}'.format(np.mean(list_ndsall)))
#print('ERR vals at this epoch = {}, {}, {}\n'.format(err_list[0],err_list[1],err_list[2]))


# In[67]:
nd_arr = np.asarray(list_ndsall)
