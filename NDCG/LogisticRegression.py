...
######## ---------- LOADING THE DATA ---------- ########
def loader():
    path = '/Users/user/Documents/ML/ndcg/MSLR-WEB10K/'
    train = datasets.load_svmlight_file(path + 'Fold1/train.txt', multilabel=False, query_id=True)
    test = datasets.load_svmlight_file(path + 'Fold1/test.txt', multilabel=False, query_id=True)
    valid = datasets.load_svmlight_file(path + 'Fold1/vali.txt', multilabel=False, query_id=True)
...
def all_rank(qd_col,output,true ):
    ...
    for i in range(len(qd_col)):
        ...
        _, _, _, _, _, f  = zip(*sorted(zip(pred_qid_rel, sec_rel, third_rel,fourth_qid, fith_qid,true_qid_rel)))
...
def total_dcg(truth, pred, count):
    ndcg = []
    for i in range(len(truth)):
        true_dcg = dcg_calc(truth[i], count)
        pred_dcg = dcg_calc(pred[i], count)
        y = (pred_dcg/true_dcg)
...
def main():
    ...
    ndcg = total_dcg(true_order, pred_ord, 10)
    print("NDCG@10 mean =", np.mean(ndcg))
    print("NDCG@10 max =",np.max(ndcg))
    print("NDCG@10 min =",np.min(ndcg))
    print("NDCG@10 std =",np.std(ndcg))

    ndcg = total_dcg(true_order, pred_ord, 0)
    print("NDCG@all mean =", np.mean(ndcg))
    print("NDCG@all max =",np.max(ndcg))
    print("NDCG@all min =",np.min(ndcg))
    print("NDCG@all std =",np.std(ndcg))
